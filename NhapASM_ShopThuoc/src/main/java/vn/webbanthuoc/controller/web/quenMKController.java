...
public class quenMKController extends HttpServlet {
    ...
    private void quenMK(HttpServletRequest request, HttpServletResponse response)
            throws IllegalAccessException, InvocationTargetException, ServletException, IOException, MessagingException {
        ...
        if (request.getMethod().equalsIgnoreCase("POST")) {
            if (request.getParameter("buttonxacthuc") != null) {
                ...
                if (sessionMaXacThuc != null && sessionMaXacThuc.equals(maXacThuc)) {
                    ...
                    if (isValidPassword(matKhauMoi)) {
                        ...
                        if (khachHang != null) {
                            ...
                            try {
                                ...
                                request.setAttribute("error", "Lỗi khi cập nhật mật khẩu Khách Hàng: " + e.getMessage());
                            }
                    ...
                    request.setAttribute("error", "Mã xác thực không đúng hoặc đã hết hạn. Vui lòng kiểm tra lại hoặc yêu cầu lại mã.");
                }
            }
        }
        request.getRequestDispatcher("/views/web/xacthuc.jsp").forward(request, response);
